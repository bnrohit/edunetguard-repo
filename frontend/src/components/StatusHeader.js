...
import { RefreshCw, Clock, Shield, Wifi, Database } from 'lucide-react';
        ...
        <div className="flex items-center justify-between flex-wrap gap-4">
          <div className="flex items-center gap-3 min-w-0">
            ...
            <div className="min-w-0">
              <div className="flex items-center gap-2 flex-wrap">
                ...
                <span className="hidden sm:inline-flex px-2 py-0.5 rounded-full bg-blue-50 text-blue-700 border border-blue-100 text-[10px] font-bold uppercase tracking-wider">
                  Network Continuity
                </span>
              </div>
          ...
          <div className="flex items-center gap-3 sm:gap-4">
            <div className={`hidden md:flex items-center gap-2 px-3 py-1.5 rounded-full border text-xs font-semibold ${
              ...
              {demoMode ? <Database className="w-3.5 h-3.5" /> : <Wifi className="w-3.5 h-3.5" />}
              {demoMode ? 'Demo data' : connected ? 'Live monitoring' : 'Connecting'}
            </div>
